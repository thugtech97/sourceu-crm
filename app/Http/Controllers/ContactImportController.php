...
class ContactImportController extends Controller
{
    ...
    {
        return Inertia::render('crm/contacts/import/index', [
    ...
    public function upload(Request $request): RedirectResponse
    {
        ...
        // Store file
        $filename = 'contact-import-'.now()->timestamp.'-'.uniqid().'.'.$extension;
        ...
        return to_route('contacts.import.mapping');
    }

    public function mapping(): InertiaResponse
    {
        $fileData = session('import_file_data');
        if (! $fileData) {
            return redirect()->route('contacts.import.index');
        }

        $defaultMappings = $this->getDefaultColumnMappings($fileData['headers']);

        return Inertia::render('crm/contacts/import/mapping', [
    ...
    public function confirm(Request $request): RedirectResponse
    {
        ...
        session()->forget('import_file_data');

        return to_route('contacts.import.status', $batch->id);
    }
    ...
    public function status(ContactImportBatch $batch): InertiaResponse
    {
        ...
        return Inertia::render('crm/contacts/import/status', [
    ...
    public function downloadTemplate(): StreamedResponse
    {
        ...
        $sampleData = [
            ['John', 'Doe', 'john@example.com', '+639171234567', 'Manager', 'lead', 'Sample note', 'inbound', 'sales'],
            ['Jane', 'Smith', 'jane@example.com', '+639181234567', 'Owner', 'prospect', 'Sample note', 'cold', 'cold_calling'],
        ];
    ...
    {
        $filename = 'contact-import-template.xlsx';
    ...
    public function downloadErrorLog(ContactImportBatch $batch)
    {
        ...
        return response()->download(
            $filePath,
            'contact-import-errors-'.$batch->id.'.csv',
            [
